...
def EncoderDecoder3D(
    ...
    for i in range(len(num_channels_down)):
        ...
        conv_down = conv3d(input_depth, num_channels_down[i], filter_size_down[i], stride=(1, 2, 2), bias=need_bias, pad=pad, downsample_mode=downsample_mode[i])
...
def conv3d(in_channels, out_channels, kernel_size=(1,1,1), stride=(1,1,1), bias=True, pad='zero', downsample_mode='stride'):
    downsampler = None
    if stride != 1 and downsample_mode != 'stride':
        ...
        if downsample_mode == 'avg':
            ...
            downsampler = Downsampler(n_planes=out_channels, factor=stride, kernel_type=downsample_mode, phase=0.5, preserve_size=True)
        else:
            assert False
